...
public class iTweenPathEditor : Editor
{
	...
	public override void OnInspectorGUI(){
		...
		for (int i = 0; i < _target.nodes.Count; i++) {
			_target.nodes[i] = CalcPos(EditorGUILayout.Vector3Field("Node " + (i+1), _target.GetRealPos(i)));
		}
	...
	Vector3 CalcPos(Vector3 realPos)
	{
		return Quaternion.Inverse(_target.transform.rotation) * (realPos - _target.transform.position);
	}
